#!/usr/bin/env python3
"""Replay validation of the pinion-sourced firmware check + Python VM clip vs real rlogs.

Simulates EXACT new firmware semantics per lateral frame:
  - angle_meas: 6-sample buffer of pinion-derived curvature CAN units
    (FORD_EXPLORER_PINION_PARAMS conversion, QF-gated)
...
Also recomputes the Python-layer clip-bite rate with the VM measurement (Fix A) vs
the historical yaw measurement.
"""
import sys
import numpy as np
...
DEG_TO_CAN = 50000
MAX_ANGLE_ERR = 150       # 0.003
GATE = 10.0               # m/s
SLIP, SR, WB = -0.0005544735, 16.8, 3.025
...
def run_route(route, segs):
    ...
    eng = eng.astype(bool); sp = sp.astype(bool)
    ...
    n = len(meas_can)
    mn = np.empty(n); mx = np.empty(n)
    for i in range(n):
        lo = max(0, i - 5)
        mn[i] = meas_can[lo:i+1].min(); mx[i] = meas_can[lo:i+1].max()
    ...
    # Python layer: clip-bite with VM measurement (no offset — conservative) vs yaw
    k_vm = -np.radians(sa) / (SR * WB) / np.maximum(1 - SLIP * v * v, 0.3)
    k_yaw = -yaw / np.maximum(v, 0.1)
    des_op = des
    mm = hands_off & (np.abs(des_op) > 0.0015) & (v > 9)
    for name, k in [('vm', k_vm), ('yaw', k_yaw)]:
        clipped = (des_op > k + CURVATURE_ERROR) | (des_op < k - CURVATURE_ERROR)
        out[f'py_clip_bite_{name}_pct'] = round(100 * float(clipped[mm].mean()), 1) if mm.sum() else None
    return out


if __name__ == '__main__':
    for route, segs in [('00000001--255eabb4d0', range(0, 25)),
                        ('000000d9--89f2063192', range(0, 61, 2))]:
        print(f'\n===== {route} =====')
