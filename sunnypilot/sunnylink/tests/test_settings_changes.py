...
class TestBluePilotLateralSchemeSplit:
  ...
  def test_high_speed_dampening_contract(self, schema):
    items = schema["vehicle_settings"]["ford"]["items"]
    keys = [item["key"] for item in items]
    ...
    assert item["title"] == "[Lateral Tuning] High Speed Dampening (Angle)"
    expected_description = "".join((
      "High speed dampening is for straightaways at highway speed.  ",
      "For oversteer reduce it, For understeer increase it. Default: 1.0",
    ))
